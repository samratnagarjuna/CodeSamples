...
namespace Illehc
{
	namespace Threading
	{
		...
		DWORD WINAPI ThreadedFileProcessor::LoadThreadFunction(void * i_pThreadData)
		{
			...
			FileOverlapped.hEvent = CreateEventA(NULL, FALSE, FALSE, NULL);
			...
			while (!bDone)
			{
				...
				else if (LQResult == WAIT_OBJECT_0 + 1)
				{
					...
					if (pLoadTask)
					{
						...
						{
							Illehc::ConsolePrint("Error Opening File %s : %d", pFileName, GetLastError());
							delete pLoadTask;
						}
						else
						{
							...
							{
								Illehc::ConsolePrint("Failed reading %s : %d", pFileName, GetLastError());
								delete pLoadTask;
							}
							else
							{
								...
								{
									Illehc::ConsolePrint("Failed waiting on File I/O %s : %d", pFileName, GetLastError());
									delete pLoadTask;
								}
							}
						}
